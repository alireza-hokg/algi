import { useEffect, useState } from "react";
import { get } from "../../services/api.js";

const ProductImages = ({ product_id }) => {
    // image state
    const [selectedImage, setSelectedImage] = useState(0);
    const [image, setImage] = useState({})
    const [images, setImages] = useState([]);
    const [product, setProduct] = useState({});

    useEffect(()=> {
        const fetchData = async () => {
            try {
                const { data: productData } = await get(`products/${product_id}/images`);
                setProduct(productData?.body)
                setImages(productData?.body?.Product_Images)
                setImage(prevImage => {
                    return productData?.body?.Product_Images?.find(image=> image.is_main)
                })
            }
            catch(err) {
                console.log(err.message)
            }
        }
        fetchData();
        // setImage(prevImage => {
        //     return product?.Product_Images?.find(image=> {
        //         return image.is_main
        //     })
        // })
        // setImages(prevImages => {
        //     return product?.Product_Images.length > 0 ? product.Product_Images : []
        // })
    }, [])

    return (
        <div className="lg:w-1/2 lg:relative">
            <div className="lg:sticky lg:top-8">
                <div className="bg-gray-100 rounded-2xl overflow-hidden group max-h-8/12">
                    <img
                        src={`http://localhost:9000/uploads/${image.image_url}`}
                        // alt={product?.name}
                        className="w-full object-cover transition-transform duration-500 group-hover:scale-105"
                    />
                </div>
                {/* Thumbnails - اگر چندتا تصویر داری */}
                <div className="flex gap-2 mt-4">
                    {images.map((img, idx) => (
                        <div 
                            key={idx} 
                            onClick={()=> {
                                setSelectedImage(idx)
                            }}
                            className="w-20 h-20 rounded-lg overflow-hidden border-2 border-transparent hover:border-amber-500 transition-all cursor-pointer">
                            <img 
                                src={`http://localhost:9000/uploads/${img.image_url}`}
                                alt={img.image_text} 
                                className="w-full h-full object-cover" 
                            />
                        </div>
                    ))}
                </div>
            </div>
        </div>
    )
}
export default ProductImages;